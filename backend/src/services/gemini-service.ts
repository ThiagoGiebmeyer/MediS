...
if (IS_FAKE_ANALYSIS_MODE) {
  ...
  console.log("✅ Modo DEV de análise ativado (padrão)");
}

const ANALISE_PROMPT = IS_FAKE_ANALYSIS_MODE ? ANALISE_PROMPT_FAKE : ANALISE_PROMPT_DEV;

const FAKE_VEGETATIVE_PHASES = ["VE", "VC", "V1", "V2", "V3", "Vn"] as const;
const FAKE_REPRODUCTIVE_PHASES = ["R1", "R2", "R3", "R4", "R5", "R6", "R7", "R8"] as const;
...
const FAKE_SIGNAL_BANK: Record<string, string[]> = {
  ...
  V1: ["uma folha trifoliolada", "crescimento vegetativo inicial", "folhagem jovem"],
  V2: ["duas folhas trifolioladas", "porte baixo", "folhas bem formadas"],
  V3: ["três folhas trifolioladas", "canópia em expansão", "folhas trifolioladas visíveis"],
  Vn: ["vegetativo sem contagem precisa", "folhagem em desenvolvimento", "estrutura vegetativa indefinida"],
  R1: ["flores iniciais", "botões florais", "início reprodutivo"],
  R2: ["flores abertas", "flores brancas visíveis", "transição para fase reprodutiva"],
  R3: ["vagens pequenas", "início de formação de vagens", "flores remanescentes"],
  R4: ["vagens em desenvolvimento", "enchimento inicial", "estrutura reprodutiva ativa"],
  R5: ["vagens em enchimento", "grãos se formando", "fase reprodutiva avançada"],
...
const pickByIndex = <T>(items: readonly T[], index: number) => items[index % items.length];
...
const extractSummaryField = (text: string) => {
  ...
  const extracted = extractTextBetween(summarySlice, 0, [/,\s*"sinais_observados"\s*:/i, /}\s*$/i]);
...
const buildGeminiPayload = (
  ...
  if (IS_FAKE_ANALYSIS_MODE) {
    console.warn("⚠️ Gerando respostas FAKE (GEMINI_ANALYSIS_MODE=fake): usando temperature=" + temperature);
  } else {
    console.log("ℹ️ Gerando respostas DEV (GEMINI_ANALYSIS_MODE=dev): usando temperature=" + temperature);
  }
...
const executarGemini = async (
  ...
  const parsedSummary = String(parsed?.resumo || "Sem resumo fornecido pelo modelo.").trim();
  const parsedPhase = (() => {
